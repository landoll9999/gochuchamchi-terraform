...
> 범위: 저장소의 **모든 `.tf` 파일 136개, 약 20,800줄**(외부 registry 모듈 캐시 `.terraform/`은 제외). 파일마다 블록 단위로 실제 라인 범위를 달아 설명한다. 닫는 괄호 같은 문법 줄은 블록에 묶어 다루며, 의미를 갖는 인자는 하나도 건너뛰지 않는 것을 원칙으로 했다.
