...
| 날짜 | 파일 | 내용 |
|---|---|---|
| 2026-08-05 | [2026-08-05.md](2026-08-05.md) | **저장소 위생 정리** — `contract.tf` fmt(신설 CI가 fmt를 hard-fail로 검사) / `.gitattributes` 신설 + `git add --renormalize`로 **"전 파일이 항상 M" 줄바꿈 노이즈 해소**(autocrlf 개인 설정 의존 제거) / 8/4 작업분(신규 20건 + waf-cdn.tf 삭제) 커밋 정리 / `_to_delete/` 제거 |
