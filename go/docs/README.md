...
| 날짜 | 파일 | 내용 |
|---|---|---|
| 2026-08-05 | [2026-08-05.md](2026-08-05.md) | **로그 아카이브 버킷 이름 충돌 3차 재발 → import 복구** — apply가 `BucketAlreadyOwnedByYou`로 정지. 8/4 §5.5에서 Object Lock 때문에 `state rm`으로 떼어낸 것이 §5.6·§5.8에 이어 세 번째 재발이며, **재구축으로 만들어진 새 버킷(8/5 00:32 생성)에서 같은 어긋남이 다시 났다**(패턴은 재구축으로 사라지지 않음) / 새 버킷도 코드에 선언 없이 Object Lock이 켜져 있는 8/4와 동일한 불일치 — 다만 **기본 보존 규칙이 없어 객체가 잠기지 않으므로 COMPLIANCE 교착은 재발하지 않음**을 확인 / `import` 후 `0 to destroy` 확인하고 apply → 22개 생성(버킷 하위 설정 6, Firehose 파이프라인 6, CloudTrail·Flow Logs·Glue·Athena 10), 객체 909개 보존 / 재발 방지로 `prevent_destroy` 추가(트레이드오프: 재구축 시 주석 처리 또는 `state rm`→**같은 세션에서 import 복귀**) / **진단 중 오독 1건 — `list-objects-v2 --query KeyCount`의 `null`을 0으로 읽어 "빈 버킷"으로 판단**(자동 페이지네이션 시 필드가 빠짐, 실제 909개) / PowerShell이 `-out=tfplan.binary`를 `.`에서 잘라 `Too many command line arguments` — 이미 알려진 `-target=a.b` 함정을 plan 옵션에서 또 밟음 / `.gitignore`의 `*.tfplan`이 `plan -out` 산출물을 못 막던 것 보강 — **§2: ArgoCD 도메인 직접 접속 전환** — 도메인·ACM 인증서는 이미 있었고 ALB가 `internal`이라 접근 경로만 없던 것(포트포워딩의 원인). `internet-facing` + `inbound-cidrs`로 바꾸되 IP 목록은 **`endpoint_public_access_cidrs`를 재사용**해 EKS 엔드포인트와 출발지 관리를 일원화. stale `/32`의 위험이 두 배가 되는 점과 admin 비번 bcrypt 고정(B4) 필요성을 주석에 명시 ※ ALB 재생성을 수반하므로 실제 접속 검증은 apply 이후 |
