# =============================================================================
# VPC Flow Logs -> 중앙 로그 S3 + Athena 조회
#
...
resource "aws_flow_log" "vpc" {
  ...
  traffic_type         = "ALL"
  log_destination      = "${aws_s3_bucket.cloudwatch_log_archive.arn}/${local.vpc_flow_logs_s3_prefix}/"
  ...
  # delivery.logs가 버킷에 쓸 수 있는 정책이 먼저 적용돼 있어야
  # Flow Log 생성 시 전달 검증(access error)이 통과한다.
  depends_on = [aws_s3_bucket_policy.cloudwatch_log_archive]
}
